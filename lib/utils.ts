...
export const handleError = (error: unknown) => {
  if (error instanceof Error) {
    ...
    if (error.message === "User not found") {
      // Custom handling for "User not found" error
      console.log("User not found. Please handle accordingly.");
      return; // Do not throw an error for this specific case
    }

    // For other errors, throw a new Error with the original error message
...
// Data URL for the placeholder loader
...
export const download = (url: string, filename: string) => {
  if (!url) {
    handleError("Resource URL not provided! You need to provide one");
  }
